...
const Article = ({ img, title, date, link }) => {
  return (
    <motion.li 
    className="relative w-full p-4 py-6 my-4 rounded-2xl flex items-center justify-between bg-light text-dark first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:bg-dark dark:text-light dark:border-light"
    initial={{y:200}}
    whileInView={{y:0, transition: {duration: 0.5}, ease: "easeInOut"}}
    // viewport={{once: true}}
    >
      <MovingImg title={title} img={img} link={link} priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw" />
      <span className="font-semibold pl-4 text-primary dark:text-primaryDark">{date}</span>
    </motion.li>
  );
};
...
const articles = () => {
  return (
    <>
      <Head>
        <title>CodeWithSaidul || Articles</title>
      ...
      <main className="w-full mb-6 flex flex-col itmes-center justify-center overflow-hidden">
        <Layout className="pt-16">
          ...
          <div>
            {/* Featured Articles */}
            <ul className="grid grid-cols-2 gap-16 mt-5">
              {/* Featured Articles 1 */}
              <FeaturedArticle
