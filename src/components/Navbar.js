...
const Navbar = () => {
  ...
  return (
    <header className="w-full px-32 lg:px-16 md:px-12 sm:px-8 xs:px-4 py-8 font-medium flex items-center justify-between dark:text-light relative z-10">
      ...
      >
        <span
          className={`bg-dark dark:bg-light transition-all duration-300 ease-out block h-0.5 w-6 rounded-sm -translate-y-0.5 rotate-0 ${
            isOpen && "rotate-45 translate-y-1 duration-300"
          }`}
        ></span>
      ...
      {isOpen && (
        ...
        >
          ...
          <nav className="flex items-center justify-center flex-wrap gap-5 mt-3 sm:gap-1">
            <motion.a
              href="https://facebook.com/codewithsaidul1"
              target="_blank"
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.9 }}
              className="w-6"
